<script>
  // --- Konstanter för metadata och adressfält ---
  ...
  async function validateAndRenderCustomerFields() {
    // Kontrollera att bookingSettings är laddat
    if (!bookingSettings) {
      console.warn("⚠️ bookingSettings inte laddat än – avbryter validering");
      return;
    }
    ...
    if (!email || !meetingType) {
      ...
      const allFields = METADATA_KEYS.map(id => document.getElementById(id)).filter(Boolean);
      allFields.forEach(f => {
    ...
    try {
      ...
      // Använd redan hämtad data, ta bort fetchAndUpdateCustomerState
      updateCustomerFieldVisibilityAndState(data, meetingType, email);
    } catch (err) {
  ...
  function updateCustomerFieldVisibilityAndState(data, meetingType, email) {
    // Sätt clt_email och clt_meetingtype från data
    const cltEmail = document.getElementById('clt_email');
    const cltMeetingType = document.getElementById('clt_meetingtype');
    if (cltEmail) cltEmail.value = email || data.email || '';
    if (cltMeetingType) cltMeetingType.value = meetingType || data.meeting_type || '';
    ...
    const allFieldIds = METADATA_KEYS;
    ...
    // Återställ logik för att visa "Skapa"-knapp vid new_customer
    if (submitButton && data.status === 'new_customer') {
    ...
    // Visa alla required fields tills de är ifyllda, även om de redan har data
    allFields.forEach(input => {
      if (shownFields.includes(input.id) || requiredFields.includes(input.id)) {
    ...
    // Visa address_fields om något av adresselementen krävs i shownFields eller requiredFields
    const addressWrapper = document.getElementById('address_fields');
    if (addressWrapper) {
      const shouldShowAnyAddressField = ADDRESS_FIELD_IDS.some(id =>
        shownFields.includes(id) || requiredFields.includes(id)
      );
    ...
    // Säkrare kontroll av cltContactId.value
    const cltContactIdValue = cltContactId?.value;
    const allCltFieldsFilled =
      ...
      (typeof cltContactIdValue === 'string' && cltContactIdValue.trim());
    if (!allCltFieldsFilled) {
      if (!cltContactId || !(typeof cltContactIdValue === 'string' && cltContactIdValue.trim())) {
    ...
    if (allCltFieldsFilled && allVisibleContactFieldsFilled) {
      const addressInputs = ADDRESS_FIELD_IDS.map(id => document.getElementById(id)).filter(Boolean);
      ...
      if (addressCheckPassed && cltReady) {
        ...
        window.formState = {
          ...
          metadata: Object.fromEntries(METADATA_KEYS.map(k => [k, document.getElementById(k)?.value || '']))
        };
